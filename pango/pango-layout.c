 ...
 * if both @force_fit and @no_break_at_end are true, %BREAK_NONE_FIT
 * is returned.
 *
...
retry_break:
  ...
  if ((force_fit && (break_num_chars < item->num_chars || !no_break_at_end)) ||
      break_width + break_extra_width <= state->remaining_width)       /* Successfully broke the item */
    {
