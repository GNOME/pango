  ...
  for (i = 0; !done ; i++)
    {
      ...
      /* ---- UAX#29 Grapheme Boundaries ---- */
      {
        ...
        if (InCB_state == InCB_Consonant ||
          ...
          { /* Rule GB9c */
            if (InCB_state == InCB_Consonant_Linker &&
                _pango2_is_Indic_Conjunct_Break_Consonant(wc))
              {
                InCB_state = InCB_Consonant;
                is_grapheme_boundary = FALSE;
              }

            if (_pango2_is_Indic_Conjunct_Break_Extend(wc) &&
                !_pango2_is_Indic_Conjunct_Break_Linker(next_wc))
        ...
        if (InCB_state == InCB_Consonant &&
            !_pango2_is_Indic_Conjunct_Break_Extend(prev_wc) &&
            _pango2_is_Indic_Conjunct_Break_Linker(wc))
      ...
      script = (GUnicodeScript)g_unichar_get_script (wc);
...
#undef IS_OTHER_TERM
      ...
      if (attrs[i].is_cursor_position ||
          ...
          FALSE)
        {
          ...
          /* Rule LB19a */
          if (!_pango2_is_EastAsianWide(prev_wc) &&
              break_type == G_UNICODE_BREAK_QUOTATION)
            break_op = BREAK_PROHIBITED;

          if (break_type == G_UNICODE_BREAK_QUOTATION &&
              (!_pango2_is_EastAsianWide (next_wc) || done))
            break_op = BREAK_PROHIBITED;
          ...
          if ((prev_LB_i == 0 || !_pango2_is_EastAsianWide (prev_prev_wc)) &&
